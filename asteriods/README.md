...
- **Classic Physics:** Realistic ship rotation, screen-wrapping, and circle-to-circle collisions.
...
- **Audio Feedback:** A custom PWM sound engine generating classic arcade blips, thrust noise, and UFO warbling on a standard active buzzer.
...
| 1   | Active buzzer             | Volume controlled via PWM duty cycle.   |
...
GPIO 27 (PWM)   →  Active buzzer signal
```
...
| **Fire button**           | Shoot bullet in facing direction                    |
...
- Hardware abstractions (`display.cpp` for OLED, `input.cpp` for smoothing/ADC, `sound.cpp` for PWM audio).
